...
const checkIfUserIsAlredyRegistered = (user: IUser, event: IEvent) => {
  const convertedVonluteersId = event.volunteers.map((volunteer) =>
    volunteer.toString(),
  );
...
const checkIfUserHasRequiredSkill = (user: IUser, event: IEvent) => {
  const convertedRequiredSkills = event.requiredSkills.map((skill) =>
    skill.toString(),
  );
  const allUserSkillsIds = user.skills.map((skill) => skill._id.toString());
  return allUserSkillsIds.some((skill) =>
    convertedRequiredSkills.includes(skill),
  );
};
...
export const addUserToEvent = async (req: Request, res: Response) => {
  ...
  if (event && user) {
    ...
    if (!checkIfUserHasRequiredSkill(user, event)) {
      return res
        .status(401)
        .json({ error: 'User do not have required skills to this event' });
    }

    if (checkIfUserIsAlredyRegistered(user, event)) {
      return res
        .status(401)
        .json({ error: 'User is alredy registered to this event' });
    }

    const updatedEventVolunteers = [...event.volunteers, user._id];
    await Event.findByIdAndUpdate(
      eventId,
      { volunteers: updatedEventVolunteers },
      { new: true },
    );
